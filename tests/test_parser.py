...
def test_temperatures_are_ordered(parsed_df):
    mask = (parsed_df.temperature_min <= parsed_df.temperature) & (parsed_df.temperature <= parsed_df.temperature_max)
...
def test_old_datetime_column_is_dropped_after_parsing_timestamps(column_name, parsed_timestamps):
...
def test_final_df_min_timestamp(parsed_df_with_timestamps):
    assert parsed_df_with_timestamps.timestamp.min() == pd.Timestamp("2022-03-11 00:15:00")

def test_final_df_max_timestamp(parsed_df_with_timestamps):
    assert parsed_df_with_timestamps.timestamp.max() == pd.Timestamp("2022-03-12 14:45:00")
